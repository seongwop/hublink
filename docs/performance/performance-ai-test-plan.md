...
```
k6 / delivery-service 내부 테스트 API
...
즉, k6는 `delivery-service`의 내부 테스트 API를 호출하고, delivery-service는 `DeadlineRequestedEvent`를 `deadline:requested:stream`에 발행한다.
...
- delivery-service 내부 테스트 API를 통해 AI 요청 이벤트를 대량 주입할 수 있는지 확인
...
ai-service가 완전히 중지된 동안 delivery-service 내부 테스트 API로 새로 주입한 메시지는 보통 pending이 아니라 backlog로 쌓인다.
...
```
1. delivery-service 내부 테스트 API로 requested stream에 이벤트가 쌓이는지
...
| 도구 | URL 또는 위치 | 확인 항목 |
| --- | --- | --- |
| Grafana | `http://34.64.89.47:3000` | ai-service CPU, memory, JVM, DB, Redis 지표 |
...
| Zipkin | `http://34.64.89.47:9411` | ai-service 처리 trace |
...
| service logs | domain-b-vm 또는 delivery-service 실행 VM | delivery-service 내부 API 호출, requested stream 발행 로그 |

## 테스트 전용 API

성능 테스트에서는 k6로 delivery-service의 내부 테스트 API를 호출해 AI 요청 이벤트를 대량 주입한다.
...
| 지표 | 의미 | 확인 기준 |
| --- | --- | --- |
| http_reqs | delivery-service 내부 이벤트 주입 API 요청 수 | 테스트 총량 |
...
k6는 delivery-service 내부 테스트 API를 통해 Redis Stream에 이벤트를 주입하는 역할이고, 실제 AI 처리 완료 여부는 Redis Stream lag, pending entry, DB 처리 상태, generated stream 증가량, ai-service 로그를 함께 확인해야 한다.
...
- delivery-service 내부 테스트 API를 통해 이벤트를 주입할 수 있는지 확인
...
| 증상 | 개선 후보 |
| --- | --- |
| delivery-service 내부 API p95 증가 | Gateway, delivery-service, Redis XADD 병목 확인 |
...
| 증상 | 개선 후보 |
| --- | --- |
| delivery-service API 실패율 증가 | 내부 테스트 API 병목, Redis XADD 실패, Gateway timeout 확인 |
...
```
ai-service 중지
-> delivery-service 내부 테스트 API로 AI 요청 이벤트 주입
-> deadline:requested:stream backlog 생성
-> ai-service 복구
-> backlog 처리
```

ai-service가 중지된 동안에도 delivery-service 내부 테스트 API는 살아 있어야 한다.
...
- ai-service 장애 중 delivery-service 내부 테스트 API가 정상적으로 이벤트를 발행하는지 확인
...
```
ai-service 중지
-> delivery-service 내부 테스트 API 호출
...
Redis Stream pending과 stream length는 Prometheus에 exporter가 연결되어 있지 않다면 redis-cli를 기준으로 기록한다.
...
3. delivery-service 내부 테스트 API로 requested stream 이벤트 주입 확인
